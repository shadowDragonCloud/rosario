...
fn fill_basic_info_value(book: &mut Book, info_type: &str, mut info_text_texts: Vec<String>, mut info_a_texts: Vec<String>) {
...
fn parse_score_rating_num(div_rating_wrap: ElementRef, score: &mut Score, book: &Book) -> anyhow::Result<()> {
...
fn parse_score_rating_people(div_rating_wrap: ElementRef, score: &mut Score, book: &Book) -> anyhow::Result<()> {
...
fn parse_score_star_percent(div_rating_wrap: ElementRef, score: &mut Score, book: &Book) -> anyhow::Result<()> {
        ...
        match sibling.value() {
            Node::Element(_) => {
                ...
                for text in texts.iter() {
                    ...
                    match star_value {
                        Ok(star_value) => {
                            let fill_succ = fill_star_value(score, star_value, star_desc.as_str(), book.location.as_str());
                            if !fill_succ {
...
fn parse_related_info(document: &Html, book: &mut Book) -> anyhow::Result<()> {
    ...
    if let Some(div_related_info) = document.select(&div_related_info_selector).next() {
        ...
        if book_id.is_empty() {
            warn!("parse related info, book_id is empty, book_page_url= {:?}", book.location);
        }
