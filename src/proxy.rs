...
fn store_proxies(proxy_infos: &[ProxyInfo]) -> anyhow::Result<()> {
    ...
    if path::Path::new(&current_dir.join(PROXY_FILE)).is_file() {
        ...
        fs::rename(current_dir.join(PROXY_FILE), current_dir.join(PROXY_FILE_OLD))?;
    }

    let mut file = fs::OpenOptions::new().create(true).append(true).open(PROXY_FILE)?;
...
fn load_proxies() -> anyhow::Result<Vec<ProxyInfo>> {
    ...
    for line in content.lines() {
        ...
        if blocks.len() != 2 {
            warn!("load proxies error, blocks count is not 2, line= {:?}", line);
            continue;
        }
...
pub(crate) fn get_and_store_valid_proxies() -> anyhow::Result<()> {
    let kuaidaili_targets: Vec<i32> = (1..=100).collect();
    let proxy_infos = parse_kuaidaili_proxy_info(kuaidaili_targets.as_slice());
    info!("parse proxies end, bengin test proxies");
    let valid_proxy_infos: Vec<ProxyInfo> = proxy_infos.iter().filter_map(|proxy_info| {
        match test_proxy_info(proxy_info) {
            Ok(true) => {
...
pub(crate) fn get_proxy_to_use() -> anyhow::Result<ProxyInfo> {
    PROXIES.read().expect("failed to get PROXIES read lock").choose(&mut rand::thread_rng()).map(|v| (*v).clone()).ok_or_else(|| anyhow!("failed to get random proxy"))
}
...
fn test_proxy_info(proxy_info: &ProxyInfo) -> anyhow::Result<bool> {
    ...
    let client = reqwest::blocking::Client::builder().proxy(proxy).timeout(Some(std::time::Duration::from_secs(3))).build()?;
    let resp = client.get(TEST_URL).send()?;
    
    let status = resp.status();
    let text = resp.text()?;
    trace!("test prox info result, proxy_info: {:?}, response status: {:?}, response text: {:?}", proxy_info, status, text);
...
fn parse_kuaidaili_proxy_info_from_page(url: &str) -> anyhow::Result<Vec<ProxyInfo>> {
    ...
    if resp.status() != 200 {
        return Err(anyhow!("failed to parse proxy info, response is not 200, url= {:?}", url));
    }
    
    let text = resp.text().with_context(|| format!("failed to parse proxy info, get text error, url= {:?}", url))?;
    ...
    match document.select(&tbody_selector).next() {
        ...
        None => {
            Err(anyhow!("failed to parse proxy info, tbody not found, url= {:?}", url))
        }
    }
}
...
fn parse_kuaidaili_proxy_info_from_tr(tr: ElementRef) -> anyhow::Result<ProxyInfo> {
    ...
    proxy_info.last_verified = parse_kuaidaili_proxy_info_from_tr_inner(tr, td_last_verified_selector)?;
...
fn parse_kuaidaili_proxy_info_from_tr_inner(tr: ElementRef, sel: Selector) -> anyhow::Result<String> {
    match tr.select(&sel).next() {
        ...
        None => {
            Err(anyhow!("failed to parse proxy info from tr, td is None, selector= {:?}", sel))
        }
    }
}
...
impl fmt::Display for ProxyInfo {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "ip: {}, port: {}, scheme: {}, last_verified: {}, anonymous: {}, position: {}",
